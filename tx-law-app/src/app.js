...
// Renders nothing visible; the QA driver reads it via document.getElementById.
const qaResult = el("div", { id: "txlaw-result", style: "display:none" });
document.body.appendChild(qaResult);
...
// Automation hook for end-to-end QA. A custom event is used instead of a bare
// global because a browser-automation driver's --eval runs in a separate JS
// context from this module, where a window property set here is not visible.
document.addEventListener("txlaw:call", (e) => {
  const box = document.getElementById("txlaw-result");
  const req = e.detail || {};
  let out;
  try {
    if (req.op === "go") { go(req.arg); out = { ok: true, screen: state.screen }; }
    else if (req.op === "state") { out = { ok: true, state: state }; }
    else if (req.op === "compare") { out = { ok: true, result: compareOutcomes(req.arg) }; }
    else { out = { ok: false, error: "unknown op: " + req.op }; }
  } catch (err) {
    out = { ok: false, error: String(err && err.message ? err.message : err) };
  }
  if (box) box.textContent = JSON.stringify(out);
});

// Also expose a global for environments where it does work (plain page scripts).
try { window.__txlaw = { go: go, state: state, compareOutcomes: compareOutcomes }; } catch (e) { /* ignore */ }
